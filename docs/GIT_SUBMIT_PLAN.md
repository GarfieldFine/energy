...
**编制说明**：远程仓库 [GarfieldFine/energy](https://github.com/GarfieldFine/energy) 已同步多人协作提交。本计划按功能清单中的 **负责人 + 预计完成时间** 梳理四人分批次提交顺序与文件范围。

**分支命名规范（全员统一）**

| 成员 | 分支格式 | 示例 |
|------|----------|------|
| 李国豪 | `feat/lgh-MMDD` | `feat/lgh-0628`、`feat/lgh-0629`、`feat/lgh-0630`、`feat/lgh-0701` |
| 李金东 | `feat/ljd-MMDD` | `feat/ljd-0629`、`feat/ljd-0630` |
| 林佳星 | `feat/ljf-MMDD` | `feat/ljf-0629`、`feat/ljf-0701` |
| 刘嘉飞 | `feat/ljf-MMDD` 或 `feat/ljf-0701` | 与林佳星 initials 区分见团队约定 |

> **注意**：勿使用无日期后缀的 `lgh`；个人分支开发完成后由李国豪 `--no-ff` 合并进 `main` 并推送。
>
> **分支纠正（2026-06-29 误建）**：早期误推的 `origin/lgh` 已删除，同名提交现保留在 **`feat/lgh-0629`**（`18804a2`）；阶段 1 补建 **`feat/lgh-0628`**（`a72ce28`）。

**当前 main 里程碑（摘要）**

| 日期 | 分支 | 说明 | 负责人 |
|------|------|------|--------|
| 06-28 | `feat/lgh-0628` | 项目骨架、健康检查、CSV 存储 | 李国豪 |
| 06-29 | `feat/lgh-0629` | JWT、Admin、工单后端、平台运维与能耗同步 | 李国豪 |
| 06-29 | `feat/ljd-0629` / `feat/ljf-0629` | 司空语料、登录前端 | 李金东 / 林佳星 |
| 06-30 | `feat/ljd-0630` / `feat/ljf-0630` | 统计 API、仪表盘前端 | 李金东 / 林佳星 |
| 06-30 | `feat/lgh-0630` | 请求访问日志 | 李国豪 |
| 07-01 | `feat/ljf-0701` | 异常说明、大屏 | 林佳星 |
| 07-01 | `feat/lgh-0701` | HTTP/未捕获异常日志 | 李国豪 |
...
|----|------|
| 主分支 | `main`，由 **李国豪** 负责合并 |
| 个人分支 | `feat/lgh-MMDD`、`feat/ljd-MMDD`、`feat/ljf-MMDD`（**必须含日期**） |
| 合并流程 | 个人分支开发 → `git merge feat/lgh-0630 --no-ff` 进 `main` → `git push origin main` |
...
#### 2D · 李国豪（06-29 增量 · 分支 `feat/lgh-0629`）

**说明**：阶段 2A 已合并进 `main` 后，同日继续补齐 **工单后端骨架**、**平台运维状态 API**、**能耗同步循环** 与 **部署配置**；在 **`feat/lgh-0629`** 分支提交后由李国豪 `--no-ff` 合并进 `main`。
...
**依赖说明（同批合并）**：`main.py` 自阶段 2A 起已注册 `stats` / `sikong` / `assistant` / `v2` 等路由；本批 **`feat/lgh-0629`** 合并时一并带上对应 router/service 文件**，保证 `uvicorn app.main:app` 可启动（李金东 / 刘嘉飞模块仍按后续阶段独立 commit 归属）。
...
backend/app/main.py                 # lifespan 初始化日志 + 注册 request_log 中间件
backend/.env.example                # LOG_LEVEL 说明
docs/GIT_SUBMIT_PLAN.md             # 分支命名与 0630 清单
```

**提交说明**

```
feat(backend): 基础请求访问日志
```
...
git checkout -b feat/lgh-0630
git add backend/app/logging_config.py backend/app/middleware/request_log.py \
        backend/app/main.py backend/.env.example docs/GIT_SUBMIT_PLAN.md
git -c user.name="李国豪" -c user.email="2542644491@qq.com" \
  commit -m "feat(backend): 基础请求访问日志"
git checkout main
git merge feat/lgh-0630 --no-ff -m "merge: 李国豪 feat/lgh-0630 请求访问日志"
git push origin feat/lgh-0630 main
```
...
#### 4C · 李国豪（1 项 · 分支 `feat/lgh-0701`）

**说明**：阶段 4 当日无其他后端条目；李国豪补齐 **HTTP 4xx/5xx 与未捕获异常** 的结构化日志（与 3C 请求访问日志配套，POC 仅写 stdout）。
...
```
backend/app/main.py                 # custom_http_exception 警告日志 + 全局 Exception handler
docs/GIT_SUBMIT_PLAN.md
```
...
```bash
git checkout main && git pull origin main
git checkout -b feat/lgh-0701
git add backend/app/main.py docs/GIT_SUBMIT_PLAN.md
git -c user.name="李国豪" -c user.email="2542644491@qq.com" \
  commit -m "feat(backend): HTTP 与未捕获异常日志"
git checkout main
git merge feat/lgh-0701 --no-ff -m "merge: 李国豪 feat/lgh-0701 异常日志"
git push origin feat/lgh-0701 main
```
...
| 2026-06-29 | 李国豪 / 李金东 / 林佳星 | 11+ | 鉴权、Admin、语料、登录；**feat/lgh-0629** 工单与平台运维 |
| 2026-06-30 | 李金东 / 林佳星 / 李国豪 | 10 | 统计 API、仪表盘；**feat/lgh-0630** 请求日志 |
| 2026-07-01 | 李金东 / 刘嘉飞 / 林佳星 / **李国豪** | 11+ | 异常检测、RAG、大屏；**feat/lgh-0701** 异常日志 |
...
git commit -m "feat(backend): 项目骨架、健康检查与能耗 CSV 存储"
git push -u origin feat/lgh-0628   # 分支名须含 feat/ 与日期，勿用 lgh
```

### 5.1b 纠正误建分支名（`lgh` → `feat/lgh-0629`）

若远程曾误推无日期分支 `lgh`，可按提交号补建规范分支并删除旧名：

```bash
# 06-29 增量提交（原 lgh 指向的 commit）
git push origin 18804a2:refs/heads/feat/lgh-0629
# 06-28 阶段 1（可选补建，指向 a72ce28）
git push origin a72ce28:refs/heads/feat/lgh-0628
git push origin --delete lgh
git branch -m lgh feat/lgh-0629
git branch -u origin/feat/lgh-0629 feat/lgh-0629
```
...
### 5.3 李国豪 · `feat/lgh-0630`（06-30 请求日志）
...
### 5.4 李国豪 · `feat/lgh-0701`（07-01 异常日志）
...
| 课设报单完整流程 | 李国豪 | **feat/lgh-0629** 已提交 work_orders 骨架；07-05 阶段 8 补齐前端联动 |
...
*文档版本：v1.4 · 分支规范统一为 `feat/{initials}-MMDD`；李国豪 0630/0701 日志模块已对齐*
